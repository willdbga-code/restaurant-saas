...
export function useKDSItems(restaurantId: string | undefined, onNewOrder?: () => void) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      unsubscribe = onSnapshot(
        ...
        (snap) => {
          // Detecta novos itens em tempo real para alerta sonoro
          if (!snap.metadata.hasPendingWrites) {
             snap.docChanges().forEach((change) => {
               if (change.type === "added" && onNewOrder) {
                 onNewOrder();
               }
             });
          }
